...
async function loadState() {
  ...
  // Fallback: mindestens eine Assembly
...
function renderViews() {
  ...
  // Checkbox: selected fürs Order-Tab
  panel.addEventListener('change', e => {
    const cb = e.target.closest('input[type="checkbox"][data-id]');
    if (cb) {
  ...
  // Inline-Editing: mpn, hersteller, shop, lager, benoetigt
...
function setupActions() {
  ...
  document.getElementById('savePart')?.addEventListener('click', () => {
    ...
    addPart(asm.id, {
      mpn, hersteller: manu, shop,
      status, lager: stock, benoetigt: required
    });
...
// --- Realtime-Sync mit anderen Clients --------------------------------------
...
async function setup() {
  ...
  // Realtime: wenn jemand anders speichert, übernehmen
  subscribeRealtime(remote => {
    Object.assign(state, remote);
    renderAll();
  });

  // einmalig speichern, damit Zeile existiert
  saveState();
}
